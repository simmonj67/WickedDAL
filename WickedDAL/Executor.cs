...
namespace WickedDAL
{
	...
	public sealed class SWExecutor
	{
		...
		/// <returns></returns>
		public static object ExecuteMethodAndGetResult(SqlConnection connection, SqlTransaction transaction, MethodInfo method, object[] values, bool autoCloseConnection)
		{
			if (method == null)
			{
		...
		private static void GenerateCommandParameters(SqlCommand command, MethodInfo method, object[] values, int[] indexes, SWCommandType swCommandType)
		{
		...
		/// <returns></returns>
		internal static object ExecuteCommand(SqlCommand cmd, Type retType, bool autoCloseConnection)
		{
		...
		/// <returns></returns>
		private static object ExecuteCommand(SqlCommand cmd, Type retType, bool autoCloseConnection, MissingSchemaAction missingSchemaAction)
		{
			object result = null;
			...
			lock (cmd.Connection)
			{
				...
				else if (retType == typeof(System.Data.SqlClient.SqlDataReader))
				{
				...
				else if (retType == typeof(System.Data.SqlClient.SqlDataAdapter))
				{
					SqlDataAdapter da = new SqlDataAdapter(cmd);
					da.MissingSchemaAction = missingSchemaAction;
					result = da;
				}
				else if (retType == typeof(System.Data.SqlClient.SqlCommand))
				{
					result = cmd;
				}
				else
				...
				if (autoCloseConnection && !(retType == typeof(System.Data.SqlClient.SqlDataReader)))
				{
